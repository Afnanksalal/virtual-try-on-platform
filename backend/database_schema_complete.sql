-- ========================================
-- Virtual Try-On Platform - COMPLETE DATABASE SETUP
-- Supabase PostgreSQL Schema
-- ========================================
-- This script includes:
-- 1. Base schema (profiles, garments, tryon_results)
-- 2. Metadata fields (file_size_bytes, model_version, etc.)
-- 3. Body generations table
-- 4. All indexes, policies, and triggers
-- ========================================
...
BEGIN
    ...
    DROP TRIGGER IF EXISTS update_body_generations_updated_at ON public.body_generations;
...
DROP TABLE IF EXISTS public.body_generations CASCADE;
...
-- GARMENTS TABLE (with metadata fields)
...
COMMENT ON COLUMN public.garments.metadata IS 'JSONB metadata: {
  "uploaded_by": "user_id",
...
-- TRY-ON RESULTS TABLE (with metadata fields)
CREATE TABLE public.tryon_results (
  ...
  model_version TEXT DEFAULT 'catvton-v1',
...
COMMENT ON COLUMN public.tryon_results.metadata IS 'JSONB metadata: {
...
-- BODY GENERATIONS TABLE
CREATE TABLE public.body_generations (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
COMMENT ON TABLE public.body_generations IS 'Body generation requests and results';
COMMENT ON COLUMN public.body_generations.body_type IS 'Detected body type: full_body or partial_body';
COMMENT ON COLUMN public.body_generations.user_params IS 'JSONB: {
  "ethnicity": "string",
...
CREATE INDEX idx_garments_file_size ON public.garments(file_size_bytes);
CREATE INDEX idx_garments_metadata_gin ON public.garments USING GIN (metadata);

-- TryOn Results indexes
...
CREATE INDEX idx_tryon_results_model_version ON public.tryon_results(model_version);
CREATE INDEX idx_tryon_results_processing_time ON public.tryon_results(processing_time_seconds);
CREATE INDEX idx_tryon_results_metadata_gin ON public.tryon_results USING GIN (metadata);

-- Body Generations indexes
CREATE INDEX idx_body_generations_user_id ON public.body_generations(user_id);
CREATE INDEX idx_body_generations_created_at ON public.body_generations(created_at DESC);
CREATE INDEX idx_body_generations_body_type ON public.body_generations(body_type);
CREATE INDEX idx_body_generations_user_params_gin ON public.body_generations USING GIN (user_params);
...
ALTER TABLE public.body_generations ENABLE ROW LEVEL SECURITY;
...
-- BODY_GENERATIONS POLICIES
CREATE POLICY "Users can view own body generations"
  ON public.body_generations FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own body generations"
  ON public.body_generations FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own body generations"
  ON public.body_generations FOR UPDATE
  USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own body generations"
  ON public.body_generations FOR DELETE
  USING (auth.uid() = user_id);
...
CREATE TRIGGER update_body_generations_updated_at
  BEFORE UPDATE ON public.body_generations
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();
...
BEGIN
  ...
  WHERE table_schema = 'public'
    AND table_name IN ('profiles', 'garments', 'tryon_results', 'body_generations');
  ...
  WHERE schemaname = 'public'
    AND tablename IN ('profiles', 'garments', 'tryon_results', 'body_generations');
  
  -- Count indexes
  SELECT COUNT(*) INTO index_count
  FROM pg_indexes
  WHERE schemaname = 'public'
    AND tablename IN ('garments', 'tryon_results', 'body_generations');
  ...
  RAISE NOTICE 'Table policies: % (expected: 16)', policy_count;
  RAISE NOTICE 'Indexes created: % (expected: 14)', index_count;
  RAISE NOTICE '========================================';
  
  IF table_count = 4 AND bucket_count = 5 AND policy_count = 16 THEN
...
--    - profiles
--    - garments (with metadata fields)
--    - tryon_results (with metadata fields)
--    - body_generations (NEW)
--
-- ✅ 5 Storage buckets created (all public)
-- ✅ 16 Table policies created
-- ✅ 20 Storage policies created
-- ✅ 14 Performance indexes created
-- ✅ 4 Triggers created for updated_at
--
-- FEATURES INCLUDED:
-- ✅ Metadata tracking (file sizes, model versions, processing times)
-- ✅ Body generation support
